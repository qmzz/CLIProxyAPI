...
func isAuthBlockedForModel(auth *Auth, model string, now time.Time) (bool, blockReason, time.Time) {
	...
	if auth.Disabled || auth.Status == StatusDisabled {
		...
		if isXAIAutoDisabled(auth) {
			...
			if !until.IsZero() && !until.After(now) {
				// The timed auto-disable has expired; continue below.
	...
	if model != "" {
		...
		// A route alias can have no exact model state. Fall through to the
		// auth-level cooldown rather than retrying a known exhausted credential.
	}
...
// Quota.NextRecoverAt remains authoritative even if a short NextRetryAfter was
// cleared, which prevents exhausted free-tier auths from being reselected early.
