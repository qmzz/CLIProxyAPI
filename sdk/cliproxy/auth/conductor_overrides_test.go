...
func TestManager_Update_XAIExplicitManualEnableClearsAutoDisable(t *testing.T) {
	m := NewManager(nil, nil, nil)
	auth := &Auth{ID: "xai-manual-enable@example.com.json", Provider: "xai"}
	if _, err := m.Register(context.Background(), auth); err != nil {
		t.Fatalf("register: %v", err)
	}
	m.MarkResult(context.Background(), Result{
		AuthID: auth.ID, Provider: "xai", Model: "grok-4.5", Success: false,
		Error: &Error{HTTPStatus: http.StatusForbidden, Message: "permission denied"},
	})

	// PatchAuthFileStatus sends disabled=false for a deliberate operator enable.
	if _, err := m.Update(context.Background(), &Auth{
		...
		t.Fatalf("manual enable update: %v", err)
	}
	updated, ok := m.GetByID(auth.ID)
	if !ok || updated == nil {
	...
	if updated.Disabled || updated.Status != StatusActive {
		t.Fatalf("manual enable did not take effect: disabled=%v status=%q", updated.Disabled, updated.Status)
	}
	if isXAIAutoDisabled(updated) {
		t.Fatal("manual enable should clear xAI auto-disable marker")
	}
}

func TestManager_SelectAuth_RevivesExpiredXAIAutoDisable(t *testing.T) {
	m := NewManager(nil, &FillFirstSelector{}, nil)
	m.executors["xai"] = schedulerTestExecutor{}
	auth := &Auth{ID: "xai-select-revive@example.com.json", Provider: "xai"}
	if _, err := m.Register(context.Background(), auth); err != nil {
		t.Fatalf("register: %v", err)
	}
	m.MarkResult(context.Background(), Result{
	...
	selected, err := m.SelectAuth(context.Background(), "xai", "", cliproxyexecutor.Options{})
	if err != nil {
		t.Fatalf("SelectAuth() error = %v", err)
	}
	if selected == nil || selected.ID != auth.ID {
		t.Fatalf("SelectAuth() = %v, want %q", selected, auth.ID)
	}
	if selected.Disabled {
		t.Fatal("expected expired xAI auto-disable to be revived before selection")
	}
}
