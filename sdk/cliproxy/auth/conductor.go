...
// reviveExpiredXAIAutoDisables returns credentials to the scheduler after an
// xAI 24h error window. This keeps long-running instances self-healing even
// when nobody opens the management page.
func (m *Manager) reviveExpiredXAIAutoDisables() {
	if m == nil {
		return
	}
	now := time.Now()
...
func (m *Manager) Update(ctx context.Context, auth *Auth) (*Auth, error) {
	...
	// An explicit metadata disabled=false comes from an operator action. A file
	// watcher reload before our own write normally omits that key and must retain
	// the active xAI auto-disable instead.
	manualXAIEnable := isXAIAutoDisabled(existing) && !auth.Disabled && auth.Status != StatusDisabled && authMetadataIsFalse(auth, "disabled")
	if manualXAIEnable {
	...
	} else if !manualXAIEnable && (isXAIAutoDisabled(auth) || isXAIAutoDisabled(existing)) {
	...
	xaiAuto := !manualXAIEnable && (isXAIAutoDisabled(auth) || isXAIAutoDisabled(existing))
...
func authMetadataIsFalse(auth *Auth, key string) bool {
...
func (m *Manager) pickNext(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	m.reviveExpiredXAIAutoDisables()
